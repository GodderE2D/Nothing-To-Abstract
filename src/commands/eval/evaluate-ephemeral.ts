...
export class EvaluateEphemeralCommand extends Command {
  public constructor(context: Command.Context, options: Command.Options) {
    super(context, {
      ...options,
      name: "evaluate (ephemeral)",
  ...
  public override registerApplicationCommands(registry: ContextMenuCommand.Registry) {
    registry.registerContextMenuCommand(
      (builder) =>
        builder.setName("Evaluate (ephemeral)").setType(ApplicationCommandType.Message).setDefaultMemberPermissions(0),
      {
        idHints: [],
      },
    );
